...
#to get longest wait time from all records
@app.get("/longest-waittime")
def longest_waittime():
    db = sessionLocal()
    try:
        rows = db.query(queueData).filter(queueData.waitTime != None).all()
        if not rows:
            return {"longest_wait_time": 0}
        # Get the maximum waitTime from all records
        longest_wait = max((r.waitTime for r in rows if r.waitTime is not None), default=0)
        return {"longest_wait_time": float(longest_wait)}
    finally:
        db.close()
